...
This bundle is adapted for Codex usage in `agent-basic-skill`. The reference
files are kept close to upstream, but installation and invocation should follow
the local Codex workflow in this repo rather than the original Claude/`npx skills`
packaging.
...
If `slidev` is not available globally, prefer project-local commands such as
`pnpm exec slidev dev`, `pnpm exec slidev build`, and `pnpm exec slidev export`.
